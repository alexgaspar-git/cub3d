...
/*   Updated: 2022/11/01 14:20:13 by lide             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_tex(t_data *data ,t_tex *tex, char *direction, t_list *mlc)
{
	...
	if (!tex->img)
		free_list_exit(mlc, "xpm_file_to_image function", 0);
...
t_tex	*get_texture(t_data *data, t_parsing *parsing)
{
	...
	init_tex(data, &tex[0], parsing->we, parsing->mlc);
	init_tex(data, &tex[0], parsing->so, parsing->mlc);
	init_tex(data, &tex[0], parsing->ea, parsing->mlc);
}

...
/*   Updated: 2022/11/12 18:31:46 by algaspar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	dr_square_mini(int x, int y, unsigned int color, t_cub *cub)
{
	...
	ox = (cub->player->c_x - cub->player->p_x)/2;
	oy = (cub->player->c_y - cub->player->p_y)/2;
	i = x;
...
void	dr_minimap(char **map, t_cub *cub)
{
	...
	while (y < cub->pars->y_max && y < player_y + 5)
	{
		...
		{
			if (map[y][x] == '1')
				dr_square_mini(x * cub->grid/2 + cub->player->p_mx, y * cub->grid/2 + cub->player->p_my, 0xFFFFFF, cub);
			if (map[y][x] == 'P')
				dr_square_mini(x * cub->grid/2 + cub->player->p_mx, y * cub->grid/2 + cub->player->p_my, 0x827FD2, cub);
			if (map[y][x] == '0')
				dr_square_mini(x * cub->grid/2 + cub->player->p_mx, y * cub->grid/2 + cub->player->p_my, 0xAAAAAA, cub);
			if ((map[y][x] == 'N' || map[y][x] == 'E' ||  map[y][x] == 'S' || map[y][x] == 'W'))
				dr_square_mini(x * cub->grid/2 + cub->player->p_mx, y * cub->grid/2 + cub->player->p_my, 0x128EAF, cub);
			x++;
		}
		y++;
	}
